...
| ARC-WASM-001 | G1 + PRD-WASM-001 + FOUND-001; owns DEC-032 resolution | exact runtime manifest + compliant sandbox boundary |
...
| SUP-MFA-001 | local Auth + profile + client contract | genuine AAL fixtures/Fastify MFA |
...
| OPS-HOST-001 | G6 + OPS-SOURCE-001 + user availability; resolves DEC-007 + scoped creation approval | exact empty dev host boundary |
...
| SUP-CHROME-003 | dev project/Auth/identity/digest | development deployment |
| OPS-DEPLOY-DEV-001 | hosted dev + release identity + digest | pre-production drills |
