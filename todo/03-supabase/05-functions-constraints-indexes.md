...
Implementation note (2026-07-29): six forward-only slices are present:
...
curriculum draft-module facade with a fixed root-scope position lock, and a
named draft-chapter facade with a locked parent sibling scope, ID-only replay,
and sanitized audit transitions. They are documented in
