...
Implementation note (2026-07-29): twenty-five forward-only slices are present:
...
progress next-cursor, live locked staff authorization predicates, a named
curriculum draft-module facade with a fixed root-scope position lock, a named
draft-chapter facade with a locked parent sibling scope, and a named
draft-theory-section facade with canonical module-to-chapter locking, ID-only
replay, and sanitized audit transitions, plus a named scalar
draft-exercise facade with complete marker-gated definition materialization,
stored-ID answer keys, and initial versions preserved at one, plus a named
complete draft-quiz facade with private question/option/key materialization and
the same initial root-version guarantee, plus a named scalar draft-exercise
PATCH facade with canonical ancestor locking, expected-version checks, a
single-root-update scalar tree replacement, learner-history denial, and no
invented PATCH idempotency, plus a named draft-quiz root PATCH facade with
canonical ancestor locking, partial root validation, one root/definition
version advance, retained-attempt denial, no tree mutation, and no invented
...
safe audit, and no version/tree/idempotency mutation, plus a named protected
draft-quiz authoring-read facade with canonical shared ancestor/root locks, a
minimal ordered nested stored-ID definition/key projection, empty/incomplete
draft preservation, retained-attempt access, empty-delta safe audit, and no
version/tree/idempotency mutation, plus a named draft-module PATCH facade with
locked live staff authorization, strict scalar partial validation, exact
expected-version/no-op behavior, one root-only update, and redacted audit
...
behavior, one child-root update, and redacted audit without invented replay,
plus a named published-module correction facade with one locked published root,
the exact content_correction reason, strict scalar partial validation, exact
expected-version/no-op behavior, one root-only update, a redacted correction
audit, and a redacted rooted descendant/learning preservation fingerprint
without invented replay, plus a named published-chapter correction facade with
canonical module-to-chapter locking, constrained hierarchy rechecks, strict
...
preservation fingerprint without invented replay, plus a named
theory-section publication facade with canonical module-to-chapter-to-theory
locking, validated draft readiness, state-based retry before stale-version
comparison, one lifecycle-root update, effective-published-only UUID-ordered
progress recalculation, a closed lifecycle audit, and no replay record, plus a
named exercise publication facade with canonical module-to-chapter-to-exercise
...
lifecycle audit, and no replay record, plus a named chapter publication facade
with canonical module-to-chapter locking, locked scalar/category readiness
requiring published theory/exercise/quiz children, state-based retry before
stale-version comparison, one lifecycle-root update, module-effective
UUID-ordered four-source progress recalculation including optional and archived
history candidates, a closed lifecycle audit, and no replay record, plus a
named module publication facade with one locked module root, locked
scalar/direct-published-chapter readiness, state-based retry before
stale-version comparison, one lifecycle-root update, published-direct-chapter
UUID-ordered four-source pair progress activation including archived leaf
history, a closed lifecycle audit, and no replay record.
...
[slice 24](../../docs/implementation/SUP-FUNCTIONS-001-slice-24.md), and
[slice 25](../../docs/implementation/SUP-FUNCTIONS-001-slice-25.md).
