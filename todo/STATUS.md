...
- Active implementation task: SUP-FUNCTIONS-001 (in progress) — named
  ...
  theory-section publication slices are implemented and verified; remaining
  owner families and real concurrency proof are still required.
- Last verified implementation slice: SUP-FUNCTIONS-001 theory-section
  publication facade with protected local and Docker verification
...
- [SUP-FUNCTIONS-001 complete draft-quiz slice report](../docs/implementation/SUP-FUNCTIONS-001-slice-09.md);
...
- [SUP-FUNCTIONS-001 draft-quiz root PATCH slice report](../docs/implementation/SUP-FUNCTIONS-001-slice-11.md);
  a service-role-only PATCH facade now applies one atomic root update with
...
- [SUP-FUNCTIONS-001 draft-exercise authoring-read slice report](../docs/implementation/SUP-FUNCTIONS-001-slice-13.md);
  a service-role-only facade now returns only a protected stored-ID scalar
...
- [SUP-FUNCTIONS-001 draft-module PATCH slice report](../docs/implementation/SUP-FUNCTIONS-001-slice-15.md);
  a service-role-only facade now atomically updates only allowed scalar fields
...
- [SUP-FUNCTIONS-001 published-module correction slice report](../docs/implementation/SUP-FUNCTIONS-001-slice-18.md);
  a service-role-only facade now locks only the current published root,
  accepts a closed correction envelope/reason, preserves descendants and
  learning state, returns safe no-op/version results, writes a redacted
  required-reason audit only for real changes, and passes protected
  local/Docker proof.
- [SUP-FUNCTIONS-001 published-chapter correction slice report](../docs/implementation/SUP-FUNCTIONS-001-slice-19.md);
  a service-role-only facade now locks module then chapter, accepts a closed
  correction envelope/reason, permits draft or published non-archived parents,
  preserves the root, descendants, and learning state, returns safe
  no-op/version results, writes a redacted required-reason audit only for real
  changes, and passes protected local/Docker proof.
- [SUP-FUNCTIONS-001 published-theory-section correction slice report](../docs/implementation/SUP-FUNCTIONS-001-slice-20.md);
  a service-role-only facade now locks module then chapter then theory section,
  accepts a closed correction envelope/reason, permits draft or published
  non-archived ancestors, preserves the root, siblings, assessment state, and
  learning state, returns safe no-op/version results, writes a redacted
  required-reason audit only for real changes, and passes protected
  local/Docker proof.
- [SUP-FUNCTIONS-001 theory-section publication slice report](../docs/implementation/SUP-FUNCTIONS-001-slice-21.md);
  a service-role-only facade now publishes one complete theory draft with
  canonical hierarchy and UUID-ordered progress locks, accepts all
  non-archived ancestor combinations, recalculates only effective published
  progress, retries safely by lifecycle state, and passes protected
  local/Docker proof.
