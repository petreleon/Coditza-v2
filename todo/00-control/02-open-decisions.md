...
| DEC-032 | Which outer sandbox launcher will isolate authoritative Python/WASM locally and on the selected host? | Partially resolved by [ADR 0006](../../docs/adr/0006-local-python-wasm-reference-proof.md): an exact Docker public-proof launcher exists locally, with no worker-thread/WASI/in-process/API-mounted-Docker-socket fallback. It is not an authoritative controller or hosted equivalence proof; block Python exercise publication/deployment until SUP/FAST/API/QA and hosted evidence complete | Local reference proof complete; hosted provider selection/equivalence remains OPS-HOST-001 + user approval |
