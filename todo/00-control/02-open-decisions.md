...
| DEC-007 | Which deployment topology hosts the API and private grader? | Partially resolved by [ADR 0005](../../docs/adr/0005-vercel-public-api-and-private-grader-topology.md): Vercel is the public API boundary; a separately approved private execution capability is required for the controller/sandbox. Do not select its provider, region, tier, cost, or owner yet | Public/private topology resolved before ARC-WASM-001; exact service, region, tier, cost, and owner at OPS-HOST-001 |
...
| DEC-032 | Which outer sandbox launcher will isolate authoritative Python/WASM locally and on the selected host? | ADR 0005 leaves this unresolved: do not fall back to a Node worker thread, Node WASI, an API-mounted Docker socket, an in-process runner, or Vercel Sandbox/Function isolation without a complete proof. ARC-WASM-001 must select and prove a disposable no-network/secret-free/read-only/non-root/resource-limited container, microVM, or equivalent narrow launcher; block Python exercise publication/deployment if none is available | During ARC-WASM-001; hosted provider selection remains OPS-HOST-001 + user approval |
